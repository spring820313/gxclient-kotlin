...
import gxchain.core.crypto.signature.SecP256K1KeyCurve
...
class GxcPrivateKey internal constructor(
    private val key: ECKey,
    private val base58: String = base58Encode(key)
) {

    val publicKey: GxcPublicKey = GxcPublicKey(key.pubKey)
    ...
    fun sharedSecret(publicKey: GxcPublicKey): ByteArray {
        val buf =  publicKey.getKey().pubKeyPoint.multiply(this.key.privKey).normalize().xCoord.encoded
